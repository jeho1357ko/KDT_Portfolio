/**
 * 주문 내역 페이지 JavaScript 모듈
 * 주문 내역 조회, 리뷰 작성 기능을 담당
 */
...
class OrderHistoryManager {
  ...
  /**
   * 주문 메인 정보를 렌더링합니다.
   * @param {HTMLElement} orderElement - 주문 요소
   * @param {Object} order - 주문 데이터
   * @param {string} statusClass - 상태 클래스
   ...
   * @param {number} productId - 상품 ID
   * @param {number} orderId - 주문 ID
   * @param {number} orderItemId - 주문 아이템 ID
   * @returns {boolean} 리뷰 작성 가능 여부
   */
 ...
 * @param {number} orderId - 주문 ID
 * @param {number} orderItemId - 주문 아이템 ID
 * @param {string} productTitle - 상품 제목
 * @param {string} productImage - 상품 이미지
